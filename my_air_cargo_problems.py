...
class AirCargoProblem(Problem):
    ...
    def actions(self, state: str) -> list:
        ...
        knowlegd_base = PropKB()

        knowlegd_base.tell(decode_state(state, self.state_map).pos_sentence()) #sets up clauses.

        for Action in self.actions_list:
            possible = True

            for clause in Action.precond_pos:
                if clause not in knowlegd_base.clauses:   # checks to see if the action viable for state
                    possible = False

            for clause in Action.precond_neg:
                if clause in knowlegd_base.clauses:   # checks to see if the action viable for state
                    possible = False

            if possible:    #if action checks out appends the action to possible_actions.
...
def air_cargo_p1() -> AirCargoProblem:
    ...
    return AirCargoProblem(cargo, planes, airports, init, goal)
...
def air_cargo_p3() -> AirCargoProblem:
    ...
    neg = [expr('At(C1,JFK)'), expr('At(C1,ATL)'), expr('At(C1,ORD)'), expr('In(C1,P1)'), expr('In(C1,P2)'),
           ...
           expr('At(C3,JFK)'), expr('At(C3,SFO)'), expr('At(C3,ORD)'), expr('In(C3,P1)'), expr('In(C3,P2)'),
           expr('At(C4,JFK)'), expr('At(C4,SFO)'), expr('At(C4,ATL)'), expr('In(C4,P1)'), expr('In(C4,P2)'),
...
if __name__ == '__main__':
    ...
    from aimacode.search import ( Node, breadth_first_search, astar_search, depth_first_graph_search,
        uniform_cost_search, greedy_best_first_graph_search, Problem,)

    problem_list = [air_cargo_p3()]

    iter = 1
    for problem in problem_list:
        print('------------------ DOING {} -----------------------'.format(iter))
        #print('Initial State for problem is {}'.format(problem.initial))
